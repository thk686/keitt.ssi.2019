...
coastline.sf <- coastline() %>% st_as_sf() %>% st_transform(crs = 2163)
coastline.atlantic <- coastline.sf %>% filter(NAME == "Atlantic")
```
...
Now we grab the national borders and internal divisions.

```{r message=FALSE, results='hide'}
nat <- nation() %>% st_as_sf() %>% st_transform(crs = 2163)
divs <- divisions() %>% st_as_sf() %>% st_transform(crs = 2163)
```
